...
namespace Noveler.Compiler
{
    internal sealed class NovelerVisitor : NovelerParserBaseVisitor<object>
    {
        ...
        public override object VisitStructure_member_declaration([NotNull] Structure_member_declarationContext context)
        {
            ...
            var variableDeclarationContext = context.variable_declaration();
            if (variableDeclarationContext != null)
            {
                var declaration = (IVariableDeclarationStatement)VisitVariable_declaration(variableDeclarationContext);
                return new StructureMemberField(declaration);
            }

            var methodDeclarationContext = context.method_declaration();
            if (methodDeclarationContext != null)
            {
                //TODO:
                var methodDeclaration = (FunctionDeclaration)VisitMethod_declaration(methodDeclarationContext);
                return new StructureMemberFunction(methodDeclaration);
            }
            ...
            throw new Exception("Unhandled strucure member declaration.");
        }
