    ...
    FunctionDeclaration OriginalDeclaration) : IQualifyable
{
    ...
    public string GetFunctionSignatureWithReturn()
    {
        if (!IsFullyDefined)
            throw new Exception("Type is not fully qualified.");
...
public static string CreateFunctionSignature(string name, params TypeDefinition[] arguments)
{
    StringBuilder sb = new();
    sb.Append(name);
    sb.Append('(');
    ...
    {
        argument.GetFullyQualifiedName(sb, sb.Length);
        sb.Append(',');
    }
